...
function execSet (tests, callback) { 
  ...
  async.waterfall([pingServer, checkDependencies], finalize)
  ...
  function checkDependencies (callback) {
    ...
    _.each(primedSet, function (test, index) {
      ...
      if (testObj.reqLogin) sessionRequired = true;
    });
    console.log(tests)
    if (sessionRequired && !sessionMet) primedSet.unshift(require(process.cwd() + '/tests')().session.login());    
    if (cartRequired && !cartMet) {
      return verifyCartContent(primedSet, callback);
    } 
      
    
    return callback(null, primedSet)
  }
